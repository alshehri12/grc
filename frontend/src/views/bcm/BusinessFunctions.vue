...
const saveFunction = async () => {
    if (!form.value.function_id || !form.value.name || !form.value.description) {
        toast.add({ severity: 'warn', summary: 'Validation', detail: 'Please fill Function ID, Name, and Description', life: 3000 })
        return
    }
    ...
    try {
        ...
        let errorMsg = 'Failed to save function'
        if (error.response?.data) {
            const data = error.response.data
            if (data.non_field_errors?.some(e => e.includes('unique')) || data.function_id?.some(e => e.includes('unique') || e.includes('exists'))) {
                errorMsg = 'This Function ID already exists. Please enter a different ID.'
            } else if (data.detail) {
